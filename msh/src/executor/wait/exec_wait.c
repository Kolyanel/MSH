...
int update_process_status(t_exec_process *pr, int status)
{
    ...
    DBG_WAIT("pid=%d status=%d updated\n",
             pr->pid,
             status);
...
int update_job_state(t_exec_job *job)
{
    ...
    DBG_WAIT("job_id=%d state=%d exit=%d\n",
             job->job_id,
             job->state,
             job->exit_code);
...
int wait_foreground(t_job_control *jc,
                    t_exec_job *job)
{
    assert(jc && job);
    ...
    if (jc->shell_tty_fd >= 0 &&
        job->pgid > 0)
    {
        if (tcsetpgrp(jc->shell_tty_fd,
                      job->pgid) < 0)
            return -1;
    }
    ...
    DBG_WAIT("FG done job=%d status=%d\n",
             job->job_id,
             status);
...
int reap_chld(t_job_control *jc)
{
    ...
    while (1)
    {
        ...
        {
            DBG_WAIT("remove completed job=%d\n",
                     job->job_id);
